...
const WalletCard = ({ wallet, onRefresh }) => {
    ...
    const isHotWallet = wallet.provider === 'SpendSync' || wallet.walletName === 'SpendSync Hot Wallet';
    const isActive = wallet.status === 'active';
    
    return (
        <div className={`glass-panel rounded-3xl overflow-hidden flex flex-col group transition-all duration-500 border border-white/5 hover:border-indigo-500/30 hover:shadow-2xl hover:shadow-indigo-500/5 ${isHotWallet ? 'bg-indigo-600/5 border-indigo-500/40 shadow-xl' : 'bg-white/[0.01]'} ${!isActive ? 'opacity-60 grayscale hover:grayscale-0 hover:opacity-100' : ''}`}>
            <div className="p-8">
                <div className="flex justify-between items-start mb-8">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${isHotWallet ? 'bg-indigo-600 text-white' : 'bg-indigo-600/10 text-indigo-400 border border-indigo-500/10'}`}>
                            <span className="material-symbols-outlined text-2xl">account_balance_wallet</span>
                        </div>
                        <div>
                            <h3 className="text-lg font-bold text-white group-hover:text-indigo-400 transition-colors">{wallet.walletName}</h3>
                            <p className={`text-[10px] font-bold uppercase tracking-widest ${isHotWallet ? 'text-indigo-300' : 'text-slate-500'}`}>
                    ...
                    <div className="relative">
                        <button 
                            ...
                            className={`p-1.5 rounded-lg hover:bg-white/5 transition-colors ${isHotWallet ? 'text-indigo-300' : 'text-slate-600'}`}
                        >
                ...
                <div className="mb-8">
                    <span className={`text-[10px] font-bold uppercase tracking-widest block mb-2 ${isHotWallet ? 'text-indigo-300/70' : 'text-slate-500'}`}>
                        {isHotWallet ? 'Available Liquidity' : 'Current Balance'}
                    </span>
                    <div className="text-4xl font-bold text-white tabular-nums font-manrope tracking-tight">
                        ${wallet.balance?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </div>
                </div>

                <div className={`grid grid-cols-2 gap-4 pt-8 border-t ${isHotWallet ? 'border-indigo-500/20' : 'border-white/5'}`}>
                    <div>
                        <span className={`text-[10px] font-bold uppercase tracking-widest block mb-2 ${isHotWallet ? 'text-indigo-300/70' : 'text-slate-500'}`}>Rewards Balance</span>
                    ...
                    <div>
                        <span className={`text-[10px] font-bold uppercase tracking-widest block mb-2 ${isHotWallet ? 'text-indigo-300/70' : 'text-slate-500'}`}>Status</span>
                        <div className={`inline-flex items-center px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest ${
                            ...
                            wallet.status === 'inactive' ? 'bg-white/5 text-slate-500' : 
                            'bg-indigo-500/10 text-indigo-400'
                        }`}>
                            {wallet.status !== 'inactive' && <span className="w-1 h-1 rounded-full bg-emerald-500 mr-2"></span>}
                            {wallet.status === 'active' ? 'Active' : wallet.status === 'inactive' ? 'Inactive' : wallet.status}
                        </div>
                    </div>
                </div>
            </div>

            <div className={`px-8 py-4 flex justify-between items-center transition-colors ${isHotWallet ? 'bg-indigo-500/10' : 'bg-white/5 group-hover:bg-white/[0.03]'}`}>
                <span className={`text-[10px] font-bold ${isHotWallet ? 'text-indigo-300' : 'text-slate-500'}`}>
                    {isHotWallet ? 'Syncing... 99.8%' : `Last active: ${new Date(wallet.updatedAt).toLocaleDateString()}`}
                </span>
                <button className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-all ${isHotWallet ? 'text-white' : 'text-indigo-400 hover:text-indigo-300'}`}>
                    {isHotWallet ? 'Trade Assets' : 'View History'}
                    <span className="material-symbols-outlined text-sm">
                        {isHotWallet ? 'swap_horiz' : 'arrow_forward'}
                    </span>
                </button>
            </div>
        </div>
        ...
        <div className="bg-[#121414] text-[#e3e2e2] font-['Work_Sans'] antialiased min-h-screen flex">
            ...
            <div className="flex-1 flex flex-col min-w-0 md:pl-64">
                ...
                <main className="p-6 md:p-10 max-w-[1600px] mx-auto w-full space-y-10">
                    ...
                    <div className="flex items-center justify-between glass-panel px-6 py-3 rounded-2xl border border-white/5 bg-white/[0.01]">
                        <div className="flex items-center gap-4">
                            <button className="bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-3">
                                <span className="material-symbols-outlined text-lg">grid_view</span> Grid
                            </button>
                            <button className="text-slate-500 hover:text-white px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-3 transition-colors">
                                <span className="material-symbols-outlined text-lg">list</span> List
                            </button>
                        </div>
                        <div className="flex items-center gap-4">
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Compare View</span>
                            <button className="w-12 h-6 bg-white/5 rounded-full relative p-1 transition-all hover:bg-white/10 group">
                                <div className="w-4 h-4 bg-slate-500 rounded-full transition-transform group-hover:scale-110"></div>
                            </button>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {wallets.map((wallet) => (
                                <WalletCard key={wallet._id} wallet={wallet} onRefresh={fetchWallets} />
                            ))}
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center justify-between">
                            ...
                            <button className="text-indigo-400 font-bold text-sm uppercase tracking-widest hover:text-indigo-300 transition-colors border-b border-indigo-500/30 pb-1">View All Activity</button>
                        </div>
            ...
            {showAddModal && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-[#1f2020] rounded-2xl max-w-md w-full p-6 border border-white/10">
